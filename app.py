...
# ── Absolute base directory (works on Render & locally) ──────────────────────
...
# ── Data Loading ─────────────────────────────────────────────────────────────
CSV_PATH = os.path.join(BASE_DIR, "AI_Solutions_Web_Log_Dataset.csv")
...
server = flask.Flask(__name__, static_folder=ASSETS_DIR)
...
def serve_assets(path):
    return flask.send_from_directory(ASSETS_DIR, path)
